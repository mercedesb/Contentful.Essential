...
namespace Contentful.Essential.Application
{
    public class DefaultContentTypeBuilder : IContentTypeBuilder
    {
        ...
        public virtual async Task CreateOrUpdateContentTypes()
        {
            ...
            foreach (var entry in ServiceLocator.Current.GetAllInstances<IContentType>())
            {
                try
                {
        ...
        protected virtual async Task Create(IContentType entry)
        {
        ...
        protected virtual async Task Update(ContentType currentContentType, IContentType entry)
        {
        ...
        protected virtual List<Field> GetFields(Type type, out string displayField)
        {
            ...
            foreach (PropertyInfo prop in props)
            {
                try
                {
                    ...
                    // currently id must be camel case or some CMA calls fail
                    string id = prop.Name.ToCamelCase();
        ...
        protected virtual bool IsField(Type type)
        {
            return type == typeof(string)
                ...
                || type == typeof(Asset)
                || typeof(IContentType).IsAssignableFrom(type)
        ...
        protected virtual string GetFieldType(Type type, out FieldLinkType? linkType)
        {
            ...
            // Entry<IContentType>
            if (type.IsGenericType && typeof(Entry<>).IsAssignableFrom(type.GetGenericTypeDefinition()) && typeof(IContentType).IsAssignableFrom(type.GetGenericArguments()[0]))
            {
                linkType = FieldLinkType.Entry;
                return SystemFieldTypes.Link;
            }
